/* { dg-do compile } */
/*  {  dg-skip-if  "test is specific to ck810fv"  { csky-*-* }  { "*" }  { "-mcpu=ck810*fv*"  }  }  */
/* { dg-options "-O2 -mvdsp-width=64 -mhard-float -mcpu=ck810fv  -D__CSKY_VDSP64__" } */
#include "csky_vdsp.h"


#define TS(i) __simd64_int ## i ## _t
#define TU(i) __simd64_uint ## i ## _t
#define FUNCS(i) funcs ## i
#define BFUNCS(i) vnor_s ## i
#define FUNCU(i) funcu ## i
#define BFUNCU(i) vnor_u ## i

TS(8) FUNCS(8) (TS(8) a, TS(8) b){
	return BFUNCS(8)(a, b);
}

/* { dg-final { scan-assembler "vnor\.8" } }*/

TS(16) FUNCS(16) (TS(16) a, TS(16) b){
	return BFUNCS(16)(a, b);
}

/* { dg-final { scan-assembler "vnor\.16" } }*/


TS(32) FUNCS(32) (TS(32) a, TS(32) b){
	return BFUNCS(32)(a, b);
}

/* { dg-final { scan-assembler "vnor\.32" } }*/


TU(8) FUNCU(8) (TU(8) a, TU(8) b){
	return BFUNCU(8)(a, b);
}

/* { dg-final { scan-assembler "vnor\.8" } }*/

TU(16) FUNCU(16) (TU(16) a, TU(16) b){
	return BFUNCU(16)(a, b);
}

/* { dg-final { scan-assembler "vnor\.16" } }*/


TU(32) FUNCU(32) (TU(32) a, TU(32) b){
	return BFUNCU(32)(a, b);
}

/* { dg-final { scan-assembler "vnor\.32" } }*/

